...
class LeaguePairUpdate(BaseModel):
    player_1_id: int
    player_2_id: int
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                SELECT
                    ...
                    COUNT(DISTINCT lm.id) FILTER (
                        WHERE lm.status = 'completed'
                    ) AS completed_matches_count
...
@app.patch("/leagues/{league_id}/pairs/{pair_id}")
def update_league_pair(
    league_id: int,
    pair_id: int,
    data: LeaguePairUpdate,
):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                LEFT JOIN league_matches lm
                  ON lm.league_id = lp.league_id
                 AND (lm.pair_a_id = lp.id OR lm.pair_b_id = lp.id)
                WHERE lp.id = %s
                  AND lp.league_id = %s
                GROUP BY lp.id;
                """,
            ...
            if not pair:
                raise HTTPException(
                    status_code=404,
                    detail="Pareja no encontrada en esta liga",
                )

            players_changed = (
                int(pair["player_1_id"]) != int(data.player_1_id)
                or int(pair["player_2_id"]) != int(data.player_2_id)
            )

            if players_changed and int(pair["completed_matches_count"] or 0) > 0:
                raise HTTPException(
                    status_code=400,
                    detail=(
                        "Esta pareja ya tiene partidos completados. "
                        "Para proteger el historial solo puedes cambiar su nombre."
                    ),
                )
            ...
            cur.execute(
                ...
                WHERE id IN (%s, %s);
                """,
                (data.player_1_id, data.player_2_id),
            )
            if len(cur.fetchall()) != 2:
                raise HTTPException(
                    status_code=404,
                    detail="Uno de los jugadores seleccionados no existe",
                )
            ...
            cur.execute(
                ...
                WHERE league_id = %s
                  AND id <> %s
                  AND (
                    (player_1_id = %s AND player_2_id = %s)
                    OR
                    (player_1_id = %s AND player_2_id = %s)
                  )
                LIMIT 1;
            ...
            if cur.fetchone():
                raise HTTPException(
                    status_code=400,
                    detail="Esa combinación de jugadores ya está inscrita",
                )
            ...
            return {
                "message": "Pareja actualizada correctamente",
                "pair": updated_pair,
            }


@app.delete("/leagues/{league_id}/pairs/{pair_id}")
def delete_league_pair(league_id: int, pair_id: int):
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                ...
                FROM league_matches
                WHERE league_id = %s
                  AND (pair_a_id = %s OR pair_b_id = %s);
                """,
                (league_id, pair_id, pair_id),
            )
            matches_count = int(cur.fetchone()["matches_count"] or 0)

            if matches_count > 0:
                raise HTTPException(
                    status_code=400,
                    detail=(
                        "No puedes eliminar esta pareja porque ya forma parte "
                        "del fixture. Puedes editarla mientras no tenga "
